...
if [ `dpkg --print-architecture` = arm64 ]; then
  make ARCH=arm64 KCFLAGS="-mcpu=cortex-a53+crc" CROSS_COMPILE=aarch64-linux-gnu- CROSS_COMPILE_COMPAT=arm-linux-gnueabihf- $config
else
...
if [ `dpkg --print-architecture` = arm64 ]; then
  make -j 3 ARCH=arm64 KCFLAGS="-mcpu=cortex-a53+crc" CROSS_COMPILE=aarch64-linux-gnu- CROSS_COMPILE_COMPAT=arm-linux-gnueabihf- bindeb-pkg
else
